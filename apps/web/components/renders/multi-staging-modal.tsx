...
interface MultiStagingModalProps {
  open: boolean;
  onClose: () => void;
  renderId: string;
...
// One Flux Pro Fill call places all the selected products into the user's
// original room photo. We POST once to /api/stage-multi; the server combines
// masks + prompts and returns a single composite + staged_image id.
export function MultiStagingModal({
  open,
  onClose,
  renderId,
  projectId,
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch('/api/stage-multi', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        renderId,
        items: selections.map((s) => ({ itemIndex: s.itemIndex, productId: s.productId })),
      }),
    })
      .then((res) => res.json())
      .then((json: { imageUrl?: string; stagedImageId?: string; error?: string }) => {
        ...
        if (json.imageUrl) {
          ...
          setError(json.error ?? 'Multi-stage failed. Try again.');
        }
      })
      .catch((err) => {
        if (cancelled) return;
  ...
  return (
    ...
    >
      <div className="w-full max-w-3xl overflow-hidden rounded-xl bg-cream">
        <div className="flex items-start justify-between gap-3 border-b border-ink/[0.06] p-5">
          <div>
            <Eyebrow>Staging {selections.length} products together</Eyebrow>
            <DisplayHeading level={3} className="mt-1">
              All in one render
            </DisplayHeading>
            <p className="mt-2 max-w-md text-[13px] text-ink-soft">
              {selections.map((s) => s.productName).join(' · ')}
            </p>
          </div>
        ...
        <div className="relative aspect-[4/3] w-full bg-paper-warm bg-grain">
          {loading ? (
            ...
            <Image
              src={result}
              alt={`${selections.length} products placed in your room`}
              fill
            ...
            <div className="absolute inset-0 grid place-items-center p-8 text-center">
              <div>
                <p className="font-display text-h3 text-ink">Couldn't place them.</p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-ink/[0.06] p-5">
          <p className="max-w-sm text-[13px] text-ink-soft">
            AI composite. Multi-product placement is approximate — Flux blends
            scale, lighting, and material match. Confirm dimensions with
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button type="button" variant="cta" onClick={() => onSuccess?.()}>
              Done
            </Button>
          </div>
        </div>
