...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // The room-grounded recommendation now lives in a separate route
    // (/api/recommend) so the UI can stage two distinct loading
    // overlays — vision-on-the-photo, then recommendation-on-the-
    // carousels. See #143.
